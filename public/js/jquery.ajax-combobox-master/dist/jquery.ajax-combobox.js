...
$.extend(AjaxComboBox.prototype, /** @lends AjaxComboBox.prototype */ {
  ...
  _setMessage: function() {
    ...
    switch (this.option.lang) {
	  ...
	  case 'fr':
		message = {
			add_btn     : 'Ajouter un bouton',
			add_title   : 'ajouter un titre',
			del_btn     : 'Supprimer un bouton',
			del_title   : 'supprimer un titre',
			next        : 'Suivant',
			next_title  : 'Suivant' + this.option.per_page + ' (Flèche droite)',
			prev        : 'Précédent',
			prev_title  : 'Précédent' + this.option.per_page + ' (Flèche gauche)',
			first_title : 'Premier (Shift + Flèche gauche)',
			last_title  : 'Dernier (Shift + Flèche droite)',
			get_all_btn : 'Get All (Down key)',
			get_all_alt : '(bouton)',
			close_btn   : 'Fermer (Tab key)',
			close_alt   : '(bouton)',
			loading     : 'chargement...',
			loading_alt : '(chargement)',
			page_info   : 'num_page_top - num_page_end of cnt_whole',
			select_ng   : 'Attention : choisissez un élément dans la liste.',
			select_ok   : 'OK : Correctly selected.',
			not_found   : 'non trouvé',
			ajax_error  : 'Une erreur est survenue sur le serveur. (ajax-combobox)'
		};
		break;
  ...
  _setElem: function(combo_input) {
    ...
    if (this.option.plugin_type == 'textarea') {
      ...
      elem.hidden = $('<input type="hidden" />')
        .attr({
          name: hidden_name,
          id: $(elem.combo_input).attr("id")
        })
        .val('');
		
		$(elem.combo_input).attr("name", $(elem.combo_input).attr("name") + "_primary_key");
		$(elem.combo_input).attr("id", $(elem.combo_input).attr("id") + "_primary_key");
    }

    // 2. 要素をHTML内に配置する
    switch (this.option.plugin_type) {
      case 'combobox':
	    var input_group = $(document.createElement('div')).addClass(this.css_class.input_group);
        ...
        input_group
          ...
          .append(elem.hidden);
		$(elem.container).append(input_group);
