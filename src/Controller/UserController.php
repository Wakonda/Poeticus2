...
class UserController extends AbstractController
{
	public function loginAction(Request $request, AuthenticationUtils $authenticationUtils, SessionInterface $session, TranslatorInterface $translator)
	{
		if($request->query->get("t") != null)
		{
			...
			$entity = $entityManager->getRepository(User::class)->findOneBy(["token" => $request->query->get("t")]);
	...
	public function showAction(Request $request, TokenStorageInterface $tokenStorage, $username)
	{
		$entityManager = $this->getDoctrine()->getManager();
		if(!empty($username))
	...
	public function createAction(Request $request, SessionInterface $session, \Swift_Mailer $mailer, TranslatorInterface $translator)
	{
		...
		if($form->isValid())
		{
			...
			$message = (new \Swift_Message('Poéticus - '.$translator->trans('user.createAccount.Registration')))
				->setFrom('poeticus@wakonda.guru', "Poéticus")
	...
	public function forgottenPasswordSendAction(Request $request, \Swift_Mailer $mailer, TranslatorInterface $translator)
	{
		...
		$message = (new \Swift_Message("Poéticus - ".$translator->trans('forgottenPassword.index.ForgotYourPassword')))
			->setFrom('poeticus@wakonda.guru', "Poéticus")
			->setTo($entity->getEmail())
			->setBody($body, 'text/html');
	...
	// Profil show
	//** Mes Poésies
	public function poemsUserDatatablesAction(Request $request, TokenStorageInterface $tokenStorage, TranslatorInterface $translator, AuthorizationCheckerInterface $authChecker, $username)
	{
		$entityManager = $this->getDoctrine()->getManager();
		...
		$entities = $entityManager->getRepository(Poem::class)->findPoemByUserAndAuhorType($iDisplayStart, $iDisplayLength, $sortByColumn, $sortDirColumn, $sSearch, $username, $tokenStorage->getToken()->getUser(), 'user');
		$iTotal = $entityManager->getRepository(Poem::class)->findPoemByUserAndAuhorType($iDisplayStart, $iDisplayLength, $sortByColumn, $sortDirColumn, $sSearch, $username, $tokenStorage->getToken()->getUser(), 'user', true);
		...
		foreach($entities as $entity)
		{
			...
			if ($authChecker->isGranted('IS_AUTHENTICATED_REMEMBERED') and $tokenStorage->getToken()->getUser()->getUsername() == $username) {
				...
				$row[] = '<a href="'.$this->generateUrl('poemuser_edit', array("id" => $entity->getId())).'" alt=""><span class="fas fa-pencil">'.$translator->trans('user.myProfile.Edit').'</span></a> / <a href="#" alt="" data-id="'.$entity->getId().'" class="delete_poem"><span class="fas fa-times">'.$translator->trans('user.myProfile.Delete').'</span></a>';
			}
	...
	public function votesUserDatatablesAction(Request $request, $username)
	{
		...
		$entities = $entityManager->getRepository(PoemVote::class)->findVoteByUser($iDisplayStart, $iDisplayLength, $sortByColumn, $sortDirColumn, $sSearch, $username);
		$iTotal = $entityManager->getRepository(PoemVote::class)->findVoteByUser($iDisplayStart, $iDisplayLength, $sortByColumn, $sortDirColumn, $sSearch, $username, true);
